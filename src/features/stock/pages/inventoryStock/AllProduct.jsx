...
function AllProduct() {
  ...
  const allMerchandise = useSelector(getAllMerchandise);

  useEffect(() => {
    if (!searchValue) {
      setListProduct(allMerchandise);
    } else {
      const filterMerchandise = allMerchandise.filter((item) => {
        if (item.medicine) {
          return (
