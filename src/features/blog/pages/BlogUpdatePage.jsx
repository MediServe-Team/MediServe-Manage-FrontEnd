...
import { updateBlogService, getBlogById } from '../blogServices';
...
export default function BlogUpdatePage() {
  ...
  useEffect(() => {
    const getDetailBlog = async () => {
      ...
      //* set value
      setPublic(result.data.visibility);
      setContent(result.data.content);
      setTitle(result.data.title);
      if (result.data.image) {
        setImagePreview({ type: 'div', value: result.data.image });
      } else {
        const listImgs = result.data.BlogImages.map((img) => img.imageUrl);
        setImages(listImgs);
      }
    };
  ...
  const handleSubmitUpdateBlog = async (e) => {
    e.preventDefault();
    if (!title) {
    ...
    //* CALL API
    const result = await updateBlogService(axiosPrivate, blogId, bodyRequest);
    if (result.status === 201 || result.status === 200) {
      toast.success('Cập nhật thành công!');
      navigate(-1);
    } else {
      toast.error('Hệ thống gặp sự cố khi cập nhật!');
      setIsCreating(false);
    }
    setIsCreating(false);
  };
  ...
  return (
    <form
      ...
      onSubmit={(e) => handleSubmitUpdateBlog(e)}
    >
      ...
      <div className="flex-1 flex justify-between items-end px-5">
        ...
        <Button type="submit" styleBtn={'solid'} size={'medium'} width={150} disabled={isCreating}>
          {isCreating ? <ClipLoader color={'#ffffff'} loading={isCreating} size={30} /> : 'Lưu'}
        </Button>
      </div>
    </form>
  );
